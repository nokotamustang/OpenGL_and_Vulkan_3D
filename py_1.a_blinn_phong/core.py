...
class Prototype:
    ...
    def get_object(self, name):
        ...
        elif name == "debug_light":
            base_object = PrototypeDebugLight(
                app=self.app,
                light_name='light'
            )
...
class Cube:
    def __init__(self, app, albedo=(1.0, 1.0, 1.0), diffuse=0.75, specular=0.25,
        ...
        self.albedo = glm.vec3(albedo)  # Ambient (Albedo)
        self.diffuse = diffuse  # Diffuse (Lambert)
        self.specular = specular  # Specular (Blinn-Phong)
    ...
    def render(self):
        ...
        self.shader_program['material.d'].value = self.diffuse
        self.shader_program['material.s'].value = self.specular
        # Render
        self.vao.render()
...
class DebugLight:
    def __init__(self, app, light_source, name: str = "debug_light"):
        self.app = app
        self.ctx = app.ctx
...
class Scene():
    ...
    def __init__(self, app):
        ...
        # Global Light
        self.app.global_light = DirectionalLight(position=(20, 20, -20),
                                                 direction=(0, 0, 0),
        ...
        # z_pos = -20.0
        # for i in range(26):
        #     self.objects.append(Cube(app, position=(-_g*2, 0, z_pos), texture="crate_0"))
        #     self.objects.append(Cube(app, position=(-_g, 0, z_pos), texture="crate_1"))
        #     self.objects.append(Cube(app, position=(0, 0, z_pos), texture="crate_2"))
        #     self.objects.append(Cube(app, position=(_g, 0, z_pos), texture="metal_0"))
        #     self.objects.append(Cube(app, position=(_g*2, 0, z_pos), texture="metal_1"))
        #     z_pos += 1.5
        ...
        self.objects.append(Cube(app, position=(_g, 0, 0), texture="metal_0", diffuse=0.4, specular=0.8))
        self.objects.append(Cube(app, position=(_g*2, 0, 0), texture="metal_1", diffuse=0.3, specular=1.0))
        self.objects.append(Cube(app, position=(_g*3, 0, 0), texture="metal_1", albedo=(1.00, 0.71, 0.29), diffuse=0.35, specular=0.95))
    ...
    def render(self):
        ...
        # Render debug lights
        if self.app.show_debug_lights:
            if self.app.show_global_light:
                self.debug_light_global.render()
            if self.app.local_light == 1.0:
                for debug_light in self.debug_light_local:
                    debug_light.render()
