...
const float LOD1 = 25.0; // 50
const float LOD2 = 50.0; // 100
const float LOD3 = 100.0; // 400
...
mat4 m_shadow_bias = mat4(0.5, 0.0, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 0.0, 0.5, 0.0, 0.5, 0.5, 0.5, 1.0);
...
const vec4 normal_vec = vec4(0.0, 1.0, 0.0, 0.0);
...
void createQuad(vec3 in_pos, mat4 x_model) {
	...
	const mat4 mvp = base * m_view_global_light;
	// const mat4 mvp = m_proj * m_view_global_light;
	...
	// const mat4 rand_y = rotationY(random(in_pos.zx) * q_PI - qq_PI); // Minor twist
	...
	gs_out.shadow_coord = m_shadow_bias * mvp * v_pos_1;
	...
	gs_out.normal = normal;
	gs_out.color_variation = fbm(in_gl_pos.xz);
	gs_out.shadow_coord = m_shadow_bias * mvp * v_pos_2;
	gs_out.shadow_coord.z += tiny;
	EmitVertex();
	...
	gs_out.normal = normal;
	gs_out.color_variation = fbm(in_gl_pos.xz);
	gs_out.shadow_coord = m_shadow_bias * mvp * v_pos_3;
	gs_out.shadow_coord.z += tiny;
	EmitVertex();
	...
	gs_out.normal = normal;
	gs_out.color_variation = fbm(in_gl_pos.xz);
	gs_out.shadow_coord = m_shadow_bias * mvp * v_pos_4;
	gs_out.shadow_coord.z += tiny;
	EmitVertex();
...
void main() {
	...
	if ((detail_level == 1) && ((int(in_pos.x * 10) % 1) == 0 || (int(in_pos.z * 10) % 1) == 0)) {
		createQuad(in_pos, model0);
	} else if ((detail_level == 2) && ((int(in_pos.x * 5) % 1) == 0 || (int(in_pos.z * 5) % 1) == 0)) {
		createQuad(in_pos, model45);
		createQuad(in_pos, modelm45);
