...
class Scene():
    ...
    def __init__(self, app):
        ...
        self.objects.append(Cube(app, position=(_g, 0, 0), texture="metal_0", roughness=0.4, metallic=0.8))
    ...
    def render(self):
        ...
        self.app.aa.aa_fbo.clear(color=(0.08, 0.16, 0.18))
        ...
        # Pass 2 - Render scene to aa framebuffer
        self.app.aa.aa_fbo.use()
        for obj in self.objects:
            if obj.can_render:
                obj.render()
        ...
        # Pass 3 - Blit aa framebuffer to screen with ctx.copy_framebuffer
        self.ctx.screen.use()  # Switch back to the screen
        self.ctx.copy_framebuffer(self.ctx.screen, self.app.aa.aa_fbo)
...
class AA():
    def __init__(self, app):
        ...
        # From mgl docs: they are optimized for use as render targets,
        # while Texture objects may not be, and are the logical choice when
        # you do not need to sample from the produced image.
